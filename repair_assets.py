...
for path, (w, h) in assets_to_generate.items():
    ...
    # Command to run: convert logo_source -resize WxH! path
    if "splash" in path:
        # Create a black canvas of size WxH, then overlay the scaled logo centered on it.
        # Scale the logo to fit within WxH with some padding (e.g. 60% of min dimension)
        logo_size = int(min(w, h) * 0.6)
        cmd = [
            "convert",
            "-size", f"{w}x{h}",
            "canvas:black",
            "(", logo_source, "-resize", f"{logo_size}x{logo_size}", ")",
            "-gravity", "center",
            "-composite",
            path
        ]
